...
const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || "http://localhost:6441";
...
function App() {
  ...
  const [subForumRequestsStatus, setSubForumRequestsStatus] =
    useState("pending");
  ...
  const dashboardCards = useMemo(
    () => [
      {
        title: "Pending Forum Requests",
        value: forumRequests.filter((r) => r.status === "pending").length,
      },
      {
  ...
  const loadForumRequests = async () => {
    const data = await apiCall(
      `/api/forum-requests?status=${forumRequestsStatus}`,
    );
  ...
  const loadSubForumRequests = async () => {
    const data = await apiCall(
      `/api/subforum-requests?status=${subForumRequestsStatus}`,
    );
  ...
  const loadForums = async () => {
    ...
    if (
      list.length > 0 &&
      (!selectedForum || !list.find((forum) => forum._id === selectedForum._id))
    ) {
  ...
  const searchThreads = async () => {
    ...
    const data = await apiCall(
      `/api/threads/search?q=${encodeURIComponent(threadQuery)}`,
    );
  ...
  const deleteThread = async () => {
    ...
    if (data) {
      ...
      setThreadResults((prev) =>
        prev.filter((item) => item._id !== selectedThread._id),
      );
    }
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (activeTab === "forumRequests" || activeTab === "dashboard")
        loadForumRequests();
      if (activeTab === "subForumRequests" || activeTab === "dashboard")
        loadSubForumRequests();
      ...
      <main className="main-panel">
        ...
        {!isAdmin && currentUser && (
          <section className="card warning">
            Your role is <strong>{currentUser.role}</strong>. Admin-only actions
            may fail for this account.
          </section>
        )}
        ...
        {activeTab === "forumRequests" && (
          <section className="card">
            ...
            <div className="inline-actions">
              <select
                value={forumRequestsStatus}
                onChange={(e) => setForumRequestsStatus(e.target.value)}
              >
            ...
            <div className="list">
              {forumRequests.map((req) => (
                <article key={req._id} className="list-item">
                  ...
                  <div className="inline-actions">
                    <button
                      onClick={() => reviewForumRequest(req._id, "approved")}
                    >
                      Approve
                    </button>
                    <button
                      className="danger"
                      onClick={() => reviewForumRequest(req._id, "rejected")}
                    >
                      Reject
                    </button>
                  </div>
        ...
        {activeTab === "subForumRequests" && (
          <section className="card">
            ...
            <div className="inline-actions">
              <select
                value={subForumRequestsStatus}
                onChange={(e) => setSubForumRequestsStatus(e.target.value)}
              >
                <option value="pending">pending</option>
                <option value="approved">approved</option>
                <option value="rejected">rejected</option>
            ...
            <div className="list">
              {subForumRequests.map((req) => (
                <article key={req._id} className="list-item">
                  ...
                  <div className="inline-actions">
                    <button
                      onClick={() => reviewSubForumRequest(req._id, "approved")}
                    >
                      Approve
                    </button>
                    <button
                      className="danger"
                      onClick={() => reviewSubForumRequest(req._id, "rejected")}
                    >
                      Reject
                    </button>
                  </div>
              ...
              <div className="list">
                {forums.map((forum) => (
                  ...
                  >
                    ...
                    <p className="small">
                      Type: {forum.type} | Active: {String(forum.isActive)}
                    </p>
                  </button>
                ))}
              </div>
              ...
              {selectedForum ? (
                <>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForumForm((p) => ({ ...p, name: e.target.value }))
                    }
                  />
              ...
              <div className="list">
                {threadResults.map((thread) => (
                  ...
                  >
                    <h3>{thread.title}</h3>
                    <p className="small">
                      Author: {thread.author?.name || "-"} | Comments:{" "}
            ...
            <div>
              ...
              {selectedThread ? (
                <>
                  ...
                  <p className="small">
                    Pinned: {String(selectedThread.isPinned)}
                  </p>
                  <div className="inline-actions">
                    <button
                      onClick={() => setThreadPinned(!selectedThread.isPinned)}
                    >
