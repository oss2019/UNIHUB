...
export default function App() {
  ...
  // Week 2: 'threads' = thread list page, 'thread-detail' = inline thread detail
  const [threadView, setThreadView] = useState(null); // null | 'threads' | 'thread-detail'
  ...
  const openSubforum = (forumId, subforumId) => {
    ...
    setSelectedThreadId(null);
    setThreadView('threads'); // Week 2: go to threads list
  };
  ...
  const openThread = ({ forumId, subforumId, threadId }) => {
    ...
    setThreadView('thread-detail'); // Week 2: go to thread detail
  };
    ...
    >
      ...
      <div className="mx-auto flex w-full max-w-7xl gap-4 px-4 pb-28 pt-6 lg:gap-6 lg:px-5">
        ...
        <main className="min-w-0 flex-1 pb-2">
          {currentView === 'home' && threadView === 'thread-detail' && selectedThread && selectedForum && selectedSubforum && (
            <ThreadDetailView
          ...
          {currentView === 'home' && threadView === 'threads' && selectedSubforum && selectedForum && (
            <ThreadsView
              isDarkMode={isDarkMode}
              forum={selectedForum}
              subforum={selectedSubforum}
      ...
      {/* PostDetailModal removed in Week 2 — thread detail is now an inline page (ThreadDetailView) */}
