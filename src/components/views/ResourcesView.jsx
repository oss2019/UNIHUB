      ...
      <div className={`p-6 rounded border ${isDarkMode ? 'bg-reddit-card-dark border-reddit-border-dark' : 'bg-white border-gray-300'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredResources.map((resource) => (
            <div key={resource.id} className={`p-4 rounded border hover:border-indigo-500 transition-all group ${isDarkMode ? 'bg-[#1a1a1b] border-reddit-border-dark' : 'bg-gray-50 border-gray-200'}`}>
