...
// All requests target VITE_API_BASE_URL (default http://localhost:6441).
import type {
  User,
  Forum,
  SubForum,
...
async function request<T>(path: string, opts: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(
      0,
      "Network error — is the backend running on " + API_BASE + "?",
    );
  }
...
export const authApi = {
  ...
  refresh: () =>
    request<{ status: string }>("/auth/refresh", {
      method: "POST",
      noRefresh: true,
    }),
  logout: () =>
...
export const userApi = {
  get: (id: string) =>
    data<{ user: User }>(request(`/api/users/${id}`)).then((d) => d.user),
  update: (id: string, body: Partial<User>) =>
    data<{ user: User }>(
      request(`/api/users/${id}`, { method: "PATCH", body }),
    ).then((d) => d.user),
  threads: (id: string, page = 1, limit = 20) =>
    data<Paginated<Thread>>(
      request(`/api/users/${id}/threads`, { query: { page, limit } }),
    ),
  onboard: (body: { branch: string; joinedSubForums: string[] }) =>
    data<{ user: User }>(
      request(`/api/users/onboard`, { method: "POST", body }),
    ).then((d) => d.user),
};
...
export const forumApi = {
  list: () =>
    data<{ forums: Forum[] }>(request("/api/forums")).then((d) => d.forums),
  get: (id: string) =>
...
export const forumRequestApi = {
  create: (body: { name: string; description?: string; type?: ForumType }) =>
    data<{ request: ForumRequest }>(
      request("/api/forum-requests", { method: "POST", body }),
    ),
  mine: () =>
    data<{ requests: ForumRequest[] }>(request("/api/forum-requests/my")).then(
      (d) => d.requests,
    ),
  list: (status?: RequestStatus) =>
    data<{ requests: ForumRequest[] }>(
      request("/api/forum-requests", { query: { status } }),
    ).then((d) => d.requests),
  review: (
    id: string,
...
export const subforumApi = {
  ...
  get: (id: string) =>
    data<{ subForum: SubForum }>(request(`/api/subforums/${id}`)).then(
      (d) => d.subForum,
    ),
  ...
  join: (id: string) =>
    request(`/api/subforums/${id}/join`, { method: "POST" }),
  leave: (id: string) =>
    request(`/api/subforums/${id}/leave`, { method: "POST" }),
  mute: (id: string) =>
    request(`/api/subforums/${id}/mute`, { method: "POST" }),
  unmute: (id: string) =>
    request(`/api/subforums/${id}/unmute`, { method: "POST" }),
};
...
export const subforumRequestApi = {
  create: (
    ...
    data<{ request: SubForumRequest }>(
      request(`/api/forums/${forumId}/subforum-requests`, {
        method: "POST",
        body,
      }),
    ),
  mine: () =>
    data<{ requests: SubForumRequest[] }>(
      request("/api/subforum-requests/my"),
    ).then((d) => d.requests),
  list: (status?: RequestStatus) =>
    data<{ requests: SubForumRequest[] }>(
      request("/api/subforum-requests", { query: { status } }),
    ).then((d) => d.requests),
  review: (
    id: string,
    body: { status: "approved" | "rejected"; reviewNote?: string },
...
export const threadApi = {
  create: (body: CreateThreadInput) =>
    data<{ thread: Thread }>(
      request("/api/threads", { method: "POST", body }),
    ).then((d) => d.thread),
  search: (q: string, page = 1, limit = 20) =>
    data<Paginated<Thread>>(
      request("/api/threads/search", { query: { q, page, limit } }),
    ),
  bySubforum: (id: string, page = 1, limit = 20) =>
    data<Paginated<Thread>>(
      request(`/api/threads/subforums/${id}`, { query: { page, limit } }),
    ),
  byForum: (id: string, page = 1, limit = 20) =>
    data<Paginated<Thread>>(
      request(`/api/threads/forums/${id}`, { query: { page, limit } }),
    ),
  get: (id: string) =>
    data<{ thread: Thread }>(request(`/api/threads/${id}`)).then(
      (d) => d.thread,
    ),
  update: (id: string, body: Partial<Thread>) =>
    data<{ thread: Thread }>(
      request(`/api/threads/${id}`, { method: "PATCH", body }),
    ).then((d) => d.thread),
...
export const commentApi = {
  ...
  get: (id: string) =>
    data<{ comment: Comment }>(request(`/api/comments/${id}`)).then(
      (d) => d.comment,
    ),
  create: (
    threadId: string,
    ...
    if (input.attachment) {
      ...
      if (input.parentCommentId)
        fd.append("parentCommentId", input.parentCommentId);
  ...
  report: (id: string) =>
    request(`/api/comments/${id}/report`, { method: "POST" }),
};
  ...
  list: (page = 1, limit = 20, unreadOnly = false) =>
    data<{ notifications: Notification[] }>(
      request("/api/notifications", {
        query: { page, limit, unreadOnly: String(unreadOnly) },
      }),
...
export const workRequestApi = {
  create: (
    ...
    data<{ workRequest: WorkRequest }>(
      request(`/api/subforums/${sourceSubforumId}/work-requests`, {
        method: "POST",
        body,
      }),
